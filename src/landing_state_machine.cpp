...
LandingStateMachine::LandingStateMachine()
{
    ...
    // start_machine_flag = false;       // 全局状态机标志初始为false
}

// 启动状态机函数
void LandingStateMachine::StartStateMachine(mavsdk::Telemetry::PositionNed current_position, double current_altitude, double current_yaw)
{
    m_current_altitude = current_altitude; // 记录当前高度

    // 仅在首次检测到启动标志时记录位置并初始化状态
    if (1 && (start_landing_flag_ == false))
    {
        m_current_position = current_position;               // 记录当前位置为降落起始点
        m_current_yaw_deg = static_cast<float>(current_yaw); // 记录当前偏航角度
 ...
 * 更新当前检测到的地标数据
 */
void LandingStateMachine::getLandmark(const AprilTagData &landmark)
{
 ...
 */
void LandingStateMachine::getPIDOut(const PIDOutput &pid_output_)
{
 ...
 */
std::string LandingStateMachine::landingStateToString(LandingState state) const
{
    // 根据状态枚举值返回对应的字符串表示
    switch (state)
    {
 ...
 * @return 当前状态枚举值
 */
void set_landing_state_flag()
{
    // start_machine_flag = true;
}